...
Last status: SAVED 2026-09-21 – replaced failed SDPose extraction with DWPose after the pose preview showed only fragmented limbs, causing badly distorted output. `workflow_pose_character_sdxl_openpose_ipadapter_api.json` now uses DWPose -> OpenPoseXL2 ControlNet at 640x960, plus the `ComfyUI_IPAdapter_plus` custom node for the character reference. It is strictly SFW. Restart ComfyUI before loading it so DWPose is discovered. AWAITING USER LOCAL TEST.
...
12. 2026-09-21 true pose-control workflow: `workflow_pose_character_sdxl_openpose_ipadapter_api.json` uses `reference_pose.webp` and `reference_char.png` from `Packages/ComfyUI/input/`. It extracts an OpenPose skeleton with DWPose and applies `OpenPoseXL2.safetensors` at strength 0.9 for hard pose control. The `ComfyUI_IPAdapter_plus` custom node preserves the character reference. It saves `proj5_pose_map_preview_*`, which must show a complete skeleton matching the intended pose; do not use the generated image if it is fragmented. It also saves the 640x960 un-upscaled base image as `proj5_sdxl_openpose_character_base_*` and a 4x upscaled output. New components: `ControlNet/OpenPoseXL2.safetensors` (4.7GB), `StableDiffusion/sdpose_wholebody_fp16.safetensors` (1.8GB, no longer used by this workflow), `IpAdapter/ip-adapter-plus_sdxl_vit-h.safetensors` (808MB), matching `ClipVision/CLIP-ViT-H-14-laion2B-s32B-b79K.safetensors` (2.4GB), `custom_nodes/ComfyUI_IPAdapter_plus`, and `custom_nodes/comfyui_controlnet_aux`. DWPose dependencies installed in ComfyUI's venv: OpenCV, Matplotlib, scikit-image, and GPU ONNX Runtime (CUDA provider verified). DWPose downloads its detector/pose weights automatically on first queue. Restart ComfyUI before use.
